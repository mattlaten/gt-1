...
\subsection*{1.12} 
\subsubsection*{a)}
The path $P_n$ has only two automorphisms. Label the vertices $v_1, v_2, \dots, v_n$ such that $v_i$ is adjacent to $v_j$ if and only if $i = j+1$ or $j = i+1$. Then the two automorphisms are the identity permutation and the function $g : V(P_n) \to V(P_n)$ given by $f(v_i) = v_{n - i + 1}$ (that is, the path is `flipped'). Let $f$ be an automorphism. By the result proved in 1.8a, $\mathrm{deg}(f(v_1)) = \mathrm{deg}(v_1) = 1$, so $f(v_1) = v_1$ or $v_n$. If $f(v_1) = v_1$, then since $v_1$ and $v_2$ are adjacent, $f(v_1)$ and $f(v_2)$ must be adjacent also, but $f(v_1) = v_1$, $f(v_2)$ can only be $v_2$. Continuing this procedure, we get that $f$ is the identity permutation. then since $v_1$ and $v_2$ are adjacent, $f(v_1)$ and $f(v_2)$ must be adjacent also, but $f(v_1) = v_n$, $f(v_2)$ can only be $v_{n-1}$. and by continuing this procedure, we get that $f = g$.
